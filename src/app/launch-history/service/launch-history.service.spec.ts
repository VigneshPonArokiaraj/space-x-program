...
describe('LaunchHistoryService', () => {
    ...
    it('should return expected launch history (HttpClient called once)', () => {
        const launchHistoryData = [
            {
                flight_number: 1,
        ...
        httpClientSpy.get.and.returnValue(of(launchHistoryData));
        const params = '?limit=100';
        launchHistoryService.getLaunchData(params).subscribe(
            launchData => expect(launchData).toEqual(launchHistoryData, 'expected Launch Data'),
            fail
        );
